...
# --- Define local values ---
locals {
  # if using a custom domain, create aliases (root + www)
  ...
  # Tags we will reuse for everything
  common_tags = {
...
# --- S3 bucker for frontend static website
...
# Allow public access (needed for web hosting)
...
# Tell S3 how to serve the website
...
# Make the bucket objects publicy readable
...
resource "aws_apigatewayv2_api" "main" {
  name = "${local.name_prefix}-api-gatway"
  protocol_type = "HTTP"
  tags = local.common_tags
